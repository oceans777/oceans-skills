...
- 代码、架构、目录、契约或行为调整时，必须从源头解决问题，禁止只做表面最小改动，不能留下技术债，要遵循最佳实践。
...
- 每次修复或改动完成并通过匹配验证后，必须按变更意图原子化提交，并同步合入本地 `{{DEV_BRANCH}}` 后推送到远程 `origin/{{DEV_BRANCH}}`；提交、合并和推送不再等待额外用户授权，但仍不得夹带无关提交、覆盖未提交改动、推送到 `{{BASE_BRANCH}}` 或使用强制推送。
