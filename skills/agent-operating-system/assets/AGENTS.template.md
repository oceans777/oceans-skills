...
- 用户提出需求后，助手必须先查看项目代码和相关文件，再逐条罗列用户需求并请求确认，不能只根据用户提供的内容判断。
- 用户确认后，助手再开始执行；如果用户 3 分钟内没有确认，助手直接按已罗列的需求自动执行。
- 执行过程中如发现需求与项目现状不一致、存在新歧义或影响范围变化，必须再次暂停并与用户对齐。
- 结束对话前，助手必须对结果进行真实用户视角的截图验证；截图必须来自实际文件、实际页面、实际应用界面或实际产物，不能用临时渲染、拼接或片段页面冒充真实结果。
...
- 每次修复或改动完成并通过验证后，必须按项目策略安全提交、创建 PR、合并或推送；只有在项目规则或用户明确授权时，才可合并并推送本地或远程 `{{BASE_BRANCH}}`；不得夹带无关提交、覆盖未提交改动或使用强制推送。
