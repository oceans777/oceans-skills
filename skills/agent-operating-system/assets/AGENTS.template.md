...
- 除非任务明确涉及当前项目以外的示例、模板或外部仓库，否则禁止修改这些位置的 `AGENTS.md`。需要借鉴示例规则时，只能复制或迁移内容到当前仓库根目录的 `AGENTS.md`。
...
- `{{BASE_BRANCH}}` 只作为基线分支；`{{DEV_BRANCH}}` 是默认开发分支和默认推送目标。除非用户明确要求，禁止把开发改动直接提交或推送到 `{{BASE_BRANCH}}`。
- 除只读分析或用户明确说明不使用 worktree 外，所有文件修改、暂存、提交和推送都应优先在 linked git worktree 中进行；主仓库 checkout 默认只用于查看、协调和创建 worktree。
- 如果当前不在 linked git worktree，应先通过 `git worktree list` 判断现有 worktree，并优先从 `{{DEV_BRANCH}}` 为当前任务创建或切换到独立 worktree；无法创建或切换时，说明阻塞原因，不要在主仓库 checkout 中扩大修改范围。
...
- `{{BASE_BRANCH}}` 是默认基线；`{{DEV_BRANCH}}` 是默认开发分支和默认推送目标。除只读任务或用户明确说明不使用 worktree 外，改动应优先从 `{{DEV_BRANCH}}` 创建 linked worktree。
- 任务 worktree 可以使用 `{{TASK_PREFIX}}/<任务名>` 临时分支隔离实现，但完成后的合入、提交同步和远程推送目标默认是 `{{DEV_BRANCH}}`，不是 `{{BASE_BRANCH}}`；只有用户明确要求时才可推送到其他分支。
...
- 每次修复或改动完成并通过匹配验证后，必须按变更意图原子化提交到任务分支。只有用户在当前任务中明确要求同步、合入或推送时，才合入本地 `{{DEV_BRANCH}}` 并推送到 `origin/{{DEV_BRANCH}}`；仍不得夹带无关提交、覆盖未提交改动、推送到 `{{BASE_BRANCH}}` 或使用强制推送。
...
- 如果是在 `{{BASE_BRANCH}}` 上做文档维护或合并检查，必须使用项目支持的显式 allow 参数，例如 `--allow-baseline-branch` 或 `-AllowBaselineBranch`。
